...
export default class ReplaySheet extends Component {
  ...
  handleChange = async e => {
    await this.setState({ [e.target.name]: handleString(e.target.value) })
              ...
              >
                ...
                >
                  ...
                  {this.state.listCommentNO.length === 0 ? (
                    <option className='text-left' disabled={true}>
...
class DefaultDetail extends Component {
  ...
  handleChange = e => {
    ...
    let obj = list[key]
    obj['replay'][name] = handleString(e.target.value)
    this.setState({ array: list })
  }
