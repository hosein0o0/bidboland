...
export default class Moshaver extends Component {
  constructor (props) {
    super(props)
    this.sign = ''
    this.state = {
  ...
  componentDidMount () {
    if (this.state.token) {
      ...
      if (this.props.edit) {
        this.setState({
          AttachmentFile: Object.keys(this.props.moshaverAttachmentFile).map(
            _ => {
              return this.props.moshaverAttachmentFile[_]
            }
          ),
          AttachmentFileName: Object.keys(
            this.props.moshaverAttachmentFile
          ).map(_ => {
            return this.props.moshaverAttachmentFile[_]
          })
        })
      ...
      if (this.props.disabled) {
        let obj = this.props.moshaverName
        if (obj !== null) {
          this.sign = obj.sign
          this.setState({
            id: id,
  ...
  handleSubmit = status => {
    ...
    if (this.state.token && this.state.id && check) {
      if (handleCheckText(this.state.moshaverDescription)) {
        this.setState({ loading: 'submit', disabledButton: true })
        setTimeout(async () => {
          let result = Object.assign({}, this.state.AttachmentFile)
          let datareg = await new FormData()
          await datareg.append('Description', this.state.moshaverDescription)
          await datareg.append('AttachmentFile', JSON.stringify(result))
          await datareg.append(
            'AttachmentDescription',
            this.state.moshaverAttachmentDescription
              ? this.state.moshaverAttachmentDescription
              : ''
          )
          await axios({
            method: 'post',
            url: `${StaticData.domainIp}/tq/update/${this.state.id}/consultant`,
            data: datareg,
            headers: {
  ...
  handleVerify = async (status, message = '') => {
    if (this.state.token) {
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/tq/verify/${this.state.id}/consultant`,
        data: datareg,
        headers: {
          Authorization: this.state.token ? `Bearer ${this.state.token}` : null
        }
        ...
        <React.Fragment>
          <div className='w-100 row justify-content-start m-0'>
            <div className='title-password col-12'>
              <h2 className='IranSans_Bold'>نظر مشاور طرح</h2>
              <div className='line'></div>
            </div>
            <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
              <div
                className={`field-form persian textarea ${
                  this.state.foucs === 'moshaverDescription' ||
                  handleCheckText(this.state.moshaverDescription)
                    ? 'active'
                    : ''
                }`}
              >
                <div className='col p-0'>
                  ...
                  <textarea
                    className='w-100'
                    type='text'
                    name='moshaverDescription'
                    onFocus={e => this.OnFocus(e.target.name)}
                    onBlur={this.OnBlur}
                    onChange={this.handleChange}
                    value={
                      handleString(this.state.moshaverDescription)
                    }
              ...
              <div className={`field-form persian`}>
                ...
                {this.props.disabled ? (
                  <div className='pl-1 allName col row m-0 justify-content-end'>
                    {this.haldeShowName(this.state.moshaverAttachmentFile).map(
                      (data, key) => (
                        <a
                          key={key}
                          href={data}
                ...
                {!this.props.disabled ? (
                  <React.Fragment>
                    <input
                      ...
                      multiple
                      onChange={e =>
                        this.handleUpload(e, 'MoshaverFile', 'MoshaverFileName')
                      }
                    />
                    <label className='upload-label' htmlFor='upload-Moshaver'>
            ...
            <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
              <div
                className={`field-form persian ${
                  this.state.foucs === 'moshaverAttachmentDescription' ||
                  handleCheckText(this.state.moshaverAttachmentDescription)
                    ? 'active'
                    : ''
                }`}
              >
                ...
                <input
                  readOnly={this.props.disabled ? true : false}
                  type='text'
                  name='moshaverAttachmentDescription'
                  value={
                    this.state.moshaverAttachmentDescription
                      ? this.state.moshaverAttachmentDescription
                      : ''
            ...
            <div className='box-wellcome'>
              ...
              <div className='buttons-wellcome justify-content-center'>
                <button
                  ...
                  disabled={this.state.errorText === '' || this.props.disabled}
                >
