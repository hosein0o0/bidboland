...
export default class IndexTq extends Component {
  constructor (props) {
    super(props)
    this.Search = null
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - درخواست فنی`
...
class Table extends Component {
  constructor (props) {
    ...
    this.state = {
      token: Cookies.get('token'),
      header: [
        { name: 'TQR No', value: 'id' },
        { name: 'Unit', value: 'unit' },
        { name: 'Issue Date', value: 'date' },
        { name: 'Discipline', value: 'disc' },
        { name: 'Orginator', value: 'orginator' },
        { name: 'Issue by', value: 'issueby' },
        { name: "Subject's Dec.", value: 'description' },
        { name: "Reply's Dec.", value: '' },
        { name: "Handover's", value: '' },
        { name: 'Reply by FE Date', value: '' },
        { name: 'OK / WARNING', value: '' },
        { name: 'Duaration', value: '' },
        { name: 'Drawing Num.', value: '' },
        { name: 'Rev.', value: '' },
        { name: 'Status', value: '' },
        { name: 'Cause', value: '' },
        { name: 'CI', value: '' },
  ...
  handleSearch = async text => {
    if (this.state.token) {
      this.setState({ search: text.trim(), loading: 'table', textSearch: '' })
      await axios
        .get(
          `${StaticData.domainIp}/tqList/${this.state.page}?contentPerPage=${
            this.state.contentPerPage
          }&searchInAll=${text.trim()}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      ...
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/tqList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/tqList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
      await axios
        .get(url, {
          headers: {
        ...
        .catch(err => {
          this.props.handleState('role', null)
          this.setState({ loading: '' })
          if (err.response) {
            Notification.notify(Message.text(err.response.status), 'error')
          }
                ...
                this.state.row.map((data, key) => (
                  ...
                  >
                    <Row
                      ...
                      download={'TQR No'}
                      _linkDownload='tq/getZipArchive'
                    ...
                    <td className='action'>
                      {this.Permision.handlePermision(
                        ...
                        <Link to={`/${this.props.url}-${data.id}`}>
                          <span className='edit'>
                            <EditIcon />
                          </span>
                        </Link>
                      ...
                      {this.Permision.handlePermision(
                        ...
                        <a
                          target='_blank'
                          rel='noreferrer'
                          href={`${StaticData.domainIp}/tqPrint/${data.id}`}
                        >
                          <span className='edit'>
                            <PrintIcon />
                          </span>
              ...
              <div className='link-footer'>
                <a
                  href={OutputFilter.handleFilter(
                    ...
                    this.state.search,
                    `${StaticData.domainIp}/tqExport/exportExcel`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='xls'
                ...
                <a
                  href={OutputFilter.handleFilter(
                    this.state.textSearch,
                    this.state.search,
                    `${StaticData.domainIp}/tqExport/exportPdf`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='pdf'
