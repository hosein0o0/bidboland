...
export default class MainTransmittal extends Component {
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/vendorTransmittal/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      ...
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/vendorTransmittal/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/vendorTransmittal/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
      await axios
        .get(url, {
          headers: {
  ...
  GetMore = async key => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/vendorTransmittal/getDocumentList?transmittalCode=${id}`,
          {
            headers: {
              Authorization: `Bearer ${this.state.token}`
            }
                  ...
                  this.state.row.map((data, key) => (
                    ...
                    >
                      <Row
                        ...
                        _linkDownload='vendorTransmittal/getZipArchive'
                      ...
                      >
                        ...
                        this.Permision.handlePermision(
                          ...
                          <Link to={`/VendorEditTransmittal-${data.id}`}>
                            <span className='edit'>
                              <EditIcon />
                            </span>
                          </Link>
                        ...
                        {this.Permision.handlePermision(
                          ...
                          data.status === '1' ? (
                            <Link
                              to={{
                                pathname: `/vendorTransmittal-${data.id}`,
                                state: { status: 2 }
                              }}
                            >
                              <span className='edit'>
                        ...
                        this.Permision.handlePermision(
                          ...
                          <a
                            href={`${StaticData.domainIp}/vendorTransmittal/print/${data.id}`}
                            target='_blank'
                            rel='noreferrer'
                          >
                            <span className='edit'>
                        ...
                        {this.state.selectNumber === key ? (
                          <div
                            ref={`boxMore_${key}`}
                            className={`box-more ${this.handlePositionBox(
                              key
                            )}`}
                          >
                            <div
                                      ...
                                      <div className='col-12 p-0 row m-0'>
                                        <div className='col-6'>
                                          ...
                                          >
                                            ...
                                            {/* {this.state.secondData.attachment !== null ?
                                                                                            <a href={this.state.secondData.attachment['0']} target='_blank' rel='noreferrer'>مشاهده جزئیات</a>
                                                                                            : ""} */}
                                          ...
                                          <div className='item'>
                                            ...
                                            <label>{data.id}</label>
                                            <a
                                              href={`/vendorTransmittal-${data.id}`}
                                              target='_blank'
                                              rel='noreferrer'
                                            >
                                              مشاهده جزئیات
          ...
          {this.state.row.length === 0 && this.state.search === '' ? (
            ...
            <div
              className={`footer-table row mr-0 ml-0 w-100 ${
                this.state.checkVendor ? 'justify-content-end' : ''
              ...
              {!this.state.checkVendor ? (
                <div className='col-xl-5 col-lg-5 col-md-12 col-12 d-flex align-items-center'>
                  <div className='link-footer'>
                    {this.Permision.handlePermision(
                      this.state.role,
                      'builders_transmittal_export'
                    ) && (
                      <a
                        href={OutputFilter.handleFilter(
                          this.state.textSearch,
                          this.state.search,
                          `${StaticData.domainIp}/vendorTransmittal/exportExcel`
                        )}
                        target='_blank'
                        rel='noreferrer'
                        className='xls'
                    ...
                    {this.Permision.handlePermision(
                      ...
                      <a
                        href={OutputFilter.handleFilter(
                          this.state.textSearch,
                          this.state.search,
                          `${StaticData.domainIp}/vendorTransmittal/exportPdf`
                        )}
                        target='_blank'
                        rel='noreferrer'
                        className='pdf'
