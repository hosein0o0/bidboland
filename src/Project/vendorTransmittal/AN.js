...
export default class AN extends Component {
  ...
  async componentDidMount () {
    if (this.props.disabled) {
      await this.setState(this.props.detail)
    } else if (this.props.edit) {
      let state = this.props.detail
      if (state.AttachmentFile && state.NativeFile && state.DocumentFile) {
        state['AttachmentFile'] = await Object.keys(
          this.props.detail.AttachmentFile
        ).map(data => {
          return this.props.detail.AttachmentFile[data]
        })
        state['NativeFile'] = await Object.keys(
          this.props.detail.NativeFile
        ).map(_data => {
          return this.props.detail.NativeFile[_data]
        })
        state['DocumentFile'] = Object.keys(this.props.detail.DocumentFile).map(
          _data => {
            return this.props.detail.DocumentFile[_data]
          }
        )
      ...
      <div className='w-100 row m-0'>
                ...
                this.Convertor(
                  ...
                  this.Convertor(
                    ...
                    <a
                      ...
                      rel='noreferrer'
                    >
                      <span>{data.name ? data.name : data.link}</span>
                    </a>
                  ))
                )
              ) : (
        ...
        <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
          <div className={`field-form`}>
            ...
            >
              {this.props.disabled ? (
                this.Convertor(this.state.NativeFile, this.state.NativeFileName)
                  .length === 0 ? (
                  <span>موردی وجود ندارد</span>
                ) : (
                  this.Convertor(
              ...
              {this.props.disabled ? (
                ...
                this.Convertor(
                  ...
                  <span key={key}>
                    <CloseRoundedIcon
                      onClick={() =>
                        this.deleteFile(key, 'DocumentFile', 'DocumentFileName')
                      }
                    />
                    <a href={data.link} target='_blank' rel='noreferrer'>
                      <span className='p-0'>
            ...
            {this.props.disabled ? (
              ...
              <React.Fragment>
                <input
                  ...
                  multiple
                  onChange={e =>
                    this.handleUpload(e, 'DocumentFile', 'DocumentFileName')
                  }
                />
                <label className='upload-label signEnglish' htmlFor='document'>
                  {this.state.loading === 'DocumentFile' ? (
                    <Loading className='form-loader w-auto mr-2 ml-0' />
                  ) : (
                    <AttachFileIcon className='mr-2 ml-0' />
                  )}
