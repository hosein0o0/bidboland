...
export default class Accepted extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            header: [
                ...
                { name: "موضوع درس آموخته", value: 'subject_lesson' },
                { name: "ساختار شکست کار", value: 'Work_failure_structure' },
                { name: "فرآیند مربوطه", value: 'relevant_process' },
                { name: "تاریخ ارائه", value: 'date_submission' },
    ...
    handleSearch = async (text) => {
        if (this.state.token) {
            ...
            // }
            // await axios.get(`${StaticData.domainIp}/notification/archive/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`, {
            //     headers: {
    ...
    fetchData = async (text = null) => {
        if (this.state.token) {
            ...
            // let url = ''
            // if (text !== null) {
            //     url = `${StaticData.domainIp}/notification/archive/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
            // } else {
            //     url = `${StaticData.domainIp}/notification/archive/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
            // }
                                ...
                                this.state.row.length === 0 ?
                                    ...
                                    this.state.row.map((data, index) =>
                                        <tr className={this.state._active === index ? '_active' : ''} key={index} ref={`tr_${data.id}`}>
                                            ...
                                            />
                                            <td className='action'>
                                                <span className='box-toggle' onClick={() => this.setState({ _popUPText: !this.state._popUPText })}>
                                                    <VisibilityIcon className='w-auto ml-1' />
                                                    شرح درس آموخته
                                                </span>
                                                {/* {this.Permision.handlePermision(this.state.role, '') ?
                                                    <a className='mr-1 ml-1' href={data.url} target='_blank' rel='noreferrer'>
                                                        <span className='edit'><VisibilityIcon /></span>
                                                    </a>
                                                    :
                                                    ''
                                                } */}
