...
import ActionDetailedEngineering from './ActionDetailedEngineering'
// import Form from '../Form/Form'
import PopupConfirm from '../layout/popupConfirm'
export default class DetailedEngineering extends Component {
  constructor(props) {
    super(props)
  ...
  handleUrlList = () => {
    ...
    switch (status_rev) {
      ...
      case 'delete_rev':
        return {
          getList: 'getListdeleted',
          getAutoFill: 'getAutoFilldeleted',
          execl: 'exportExceldeleted',
          pdf: 'exportPdfdeleted'
        }
      default:
        return {}
    }
  ...
  fetchData = async (text = null) => {
    ...
    const { getList } = this.handleUrlList()
    if (token && handleCheckText(getList)) {
      let url = `${StaticData.domainIp}/${baseUrlAPI}/${getList}/${page || 0
        }?contentPerPage=${contentPerPage}${handleCheckText(search) ? `&searchInAll=${search}` : ''
  ...
  render() {
    ...
    let { baseUrlAPI, delete_id, loading } = this.state
      ...
      <React.Fragment>
        ...
        <div className='table w-100'>
          <table>
            ...
            <BodyTable
              {...this}
              nameDownload='detail_engineering_download'
              objLink={{
        ...
        {delete_id && (
          <PopupConfirm
            number={delete_id}
            label='تفضیلی'
            close={() => this.setState({ delete_id: null })}
            loading={loading}
            handleSubmit={() => handleDelete(delete_id, 'detailEng/delete')}
          />
        )}
      </React.Fragment>
    )
