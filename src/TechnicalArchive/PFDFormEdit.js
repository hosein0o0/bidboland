...
export default class PFDForm extends Component {
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - ویرایش PFD`
    this.GetId()
  }
  OnFocus = name => {
  ...
  fetchData = async id => {
    if (this.state.token) {
      let url = `${StaticData.domainIp}/pfd/get/${id}`
      await axios
        .get(url, {
          headers: {
  ...
  handleSubmit = async () => {
    ...
    if (check && id) {
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/pfd/update/${id}`,
        data: datareg,
        headers: {
        ...
        <div className='main'>
          <div className='col-12 p-0'>
            <div className='row m-0'>
              ...
              >
                <Menu nameRole='pfd_edit' nameUrl={this.props.nameUrl} />
