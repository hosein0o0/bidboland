...
// import { Link } freom 'react-router-dom'
// import VisibilityIcon from '@material-ui/icons/Visibility';
// import PrintIcon from '@material-ui/icons/Print';
// import GetAppRoundedIcon from '@material-ui/icons/GetAppRounded';
import EditIcon from '@material-ui/icons/Edit'
import Permision from '../permision/permision'
export default class BasicEngineeringTest extends Component {
  constructor (props) {
    super(props)
    ...
    this.state = {
      ...
      header: [
        ...
        { name: 'Document No.', value: 'document_no', download: true },
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/basicEng/getListTest/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/basicEng/getListTest/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/basicEng/getListTest/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
                ...
                this.state.row.map((data, index) => (
                  ...
                  >
                    <Row
                      ...
                      objLink={{ document_no: data.file_address }}
                      permision={this.Permision.handlePermision(
                        this.state.role,
                        'basic_engineering_download'
                      )}
          ...
          >
            <div className='col-xl-5 col-lg-5 col-md-12 col-12 d-flex align-items-center'>
              <div className='link-footer'>
                {this.Permision.handlePermision(
                  this.state.role,
                  'basic_engineering_export'
                ) && (
                  <a
                    href={OutputFilter.handleFilter(
                      this.state.textSearch,
                      this.state.search,
                      `${StaticData.domainIp}/basicEng/exportExcel`
                    )}
                    target='_blank'
                    rel='noreferrer'
                    className='xls'
                ...
                {this.Permision.handlePermision(
                  ...
                  <a
                    href={OutputFilter.handleFilter(
                      this.state.textSearch,
                      this.state.search,
                      `${StaticData.domainIp}/basicEng/exportPdf`
                    )}
                    target='_blank'
                    rel='noreferrer'
                    className='pdf'
