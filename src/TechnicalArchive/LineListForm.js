...
export default class LineListForm extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - ایحاد Linelist جدید`
  }
  OnFocus = name => {
  ...
  handleSubmit = async () => {
    ...
    if (check) {
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/lineList/create`,
        data: datareg,
        headers: {
          Authorization: this.state.token ? `Bearer ${this.state.token}` : null
        }
