...
export default class BuildersEngineeringFormEdit extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - ویرایش مهندسی سازندگان`
    this.GetId()
  }
  GetId = async () => {
  ...
  fetchData = async id => {
    if (this.state.token) {
      let url = `${StaticData.domainIp}/vpis/get/${id}`
      await axios
        .get(url, {
          headers: {
        ...
        .then(async response => {
          ...
          if (response.status === 200) {
            ...
            for (let value in state) {
              let checkDate =
                (await value) === 'scheduled_issue_date' ||
                value === 'doc_date' ||
                value === 'receiving_date' ||
                value === 'due_date' ||
                value === 'reply_date' ||
                value === 'vendor_reply_date' ||
                value === 'mc_transmittal_date' ||
                value === 'mc_tr_due_date' ||
                value === 'mc_reply_date' ||
                value === 'cons_trs_date'
              if (checkDate) {
                if (state[value]) {
  ...
  GetLink = (nameState, file, names, length, i) => {
    ...
    axios({
      method: 'post',
      url: `${StaticData.domainIp}/uploadFile/vpis`,
      data: datareg,
      headers: {
  ...
  handleSubmit = async () => {
    ...
    if (check && id) {
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/vpis/update/${id}`,
        data: datareg,
        headers: {
          Authorization: this.state.token ? `Bearer ${this.state.token}` : null
        }
        ...
        <div className='main'>
          <div className='col-12 p-0'>
            <div className='row m-0'>
              ...
              >
                <Menu
                  nameRole='builders_engineering_edit'
