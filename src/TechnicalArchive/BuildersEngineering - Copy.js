...
export default class BuildersEngineering extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      header: [
        ...
        { name: 'Discipline', value: 'disipline' },
        { name: 'Size', value: 'size' },
        { name: 'Document Description', value: 'document_description' },
        { name: 'Weight Factor', value: 'weight_factor' },
        {
          name: 'POI Status',
          value: 'poi_status',
          subHeader: { name: 'Issue for Owner/MC' }
        },
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/vpis/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/vpis/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/vpis/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
    ...
    return (
      <React.Fragment>
        <div className='table w-100'>
          <table>
            <thead>
              <tr className='header has-sub'>
                {this.state.header.map((data, index) => (
                  <th
                    key={index}
                    className={`${this.handleFilter(data.name)} ${
                      data.subHeader ? 'd-flex align-items-center border-0' : ''
                    }`}
                  ...
                  >
                    {data.subHeader ? (
                      <span className='d-inline-block'>
                        <span className='sub-head top IranSans_Bold_FA'>
                          {data.name}
                        </span>
                        <span className='sub-head _bottom IranSans_Bold_FA'>
                          {data.subHeader.name}
                        </span>
                      </span>
                    ) : (
                ...
                this.state.row.map((data, index) => (
                  ...
                  >
                    ...
                    {/* <td className='action justify-content-center'>
                                                <Link className='mr-auto ml-auto' to={`#`} target='_blank' rel='noreferrer'>
                                                    <span className='edit'><VisibilityIcon /></span>
                                                </Link>
                                                <Link className='mr-auto ml-auto' to={`#`} target='_blank' rel='noreferrer'>
                                                    <span className='edit'><PrintIcon /></span>
                                                </Link>
                                            </td> */}
                  </tr>
                ))
              ...
              <div className='link-footer'>
                <a
                  href={OutputFilter.handleFilter(
                    ...
                    this.state.search,
                    `${StaticData.domainIp}/vpis/exportExcel`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='xls'
                ...
                <a
                  href={OutputFilter.handleFilter(
                    this.state.textSearch,
                    this.state.search,
                    `${StaticData.domainIp}/vpis/exportPdf`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='pdf'
