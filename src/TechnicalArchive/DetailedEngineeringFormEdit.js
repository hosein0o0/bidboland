...
export default class DetailedEngineeringFormEdit extends Component {
  constructor(props) {
    super(props)
  ...
  fetchData = id => {
    const { fetchDataDetailEng } = this.APIForm
    fetchDataDetailEng(id)
  }
  OnFocus = name => {
  ...
  handleSubmit = pageName => {
    const { handleSubmitApi } = this.APIForm
    const { id } = this.state
    let url = `detailEng/update/${id}`
    handleSubmitApi(pageName, url)
  }
  render() {
    ...
    const { token, redirect, _close, disabled, loading, status_check, firstLoading } =
      this.state
    if (token === undefined) {
