...
export default class TableTSRRecord extends Component {
  constructor (props) {
    super(props)
    ...
    this.state = {
      ...
      header: [
        ...
        { name: 'مرحله در دست بررسی', value: '-' },
        { name: 'دلیل ویرایش', value: '-' },
        { name: 'دلیل توقف', value: '-' },
        { name: 'دلیل رد شدن', value: '-' },
  ...
  handleSearch = async text => {
    ...
    if (this.state.token) {
      ...
      if (textSearch) {
        url = `${
          StaticData.domainIp
        }/tsr/getListDraft/${page}?contentPerPage=${contentPerPage}&searchInAll=${
          text ? text : ''
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      ...
      await axios
        ...
        .then(async response => {
          if (response.status === 200) {
            const { content, userDetail } = response.data
            await this.setState({
  ...
  _handleClick = async (key, data, checkAdd) => {
    ...
    if (token) {
      ...
      if (handleCheckText(textSearch)) {
        url = `${StaticData.domainIp}/tsr/getAutoFillDraft/${nameDataBase}/page/${pageAdvance}?searchInAll=${search}&${secondUrl}`
      } else {
        url = `${StaticData.domainIp}/tsr/getAutoFillDraft/${nameDataBase}/page/${pageAdvance}?searchInAll=${search}`
      }
      await axios
        .get(url, {
          headers: {
    ...
    if (checkheader) {
      ...
      if (this.state.token) {
        ...
        if (handleCheckText(textSearch)) {
          url = `${
            StaticData.domainIp
          }/tsr/getAutoFillDraft/${nameDataBase}/page/${pageAdvance}?searchInField=${
            text ? text : ''
  ...
  CheckRejectConf = data => {
    const check =
      data.unit_boss_verify === '1' && data.office_boss_verify === '1'
    return check
  }
  JafariShow = data => {
    // 10023
    if (this.state.userDetail) {
      const { first_name, last_name } = this.state.userDetail
      const check =
        first_name === 'حیدر' && last_name === 'جعفری برزیان'
          ? this.CheckRejectConf(data) || this.CheckSignJafari(data)
          : true
      return check
    } else {
      return false
  ...
  CheckSignJafari = data => {
    const { notification_to } = data
    const check = notification_to.includes('10023')
    return check
  }
  render () {
    return (
