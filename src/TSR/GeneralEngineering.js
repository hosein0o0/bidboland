...
export default class GeneralEngineering extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - بررسی TSR توسط مهندسی عمومی`
    this.ShowFetch()
    this.props.GetShowFetch(5, this.ShowFetch)
  }
  ...
  ShowFetch = async () => {
    let tsr5 = await this.props.state.tsr5,
      obj = {}
    if (tsr5) {
      for (let value in tsr5) {
        let state
        if (value.includes('tsr5_')) {
          state = value.split('tsr5_')[1]
        } else {
          state = value
        }
        if (state === 'foreign_attachment' || state === 'internal_attachment') {
          if (this.props.show || this.props.canUpdate) {
            let objDetail = tsr5[value]
            if (objDetail) {
          ...
          obj[state] =
            tsr5[value] === '1' ? true : tsr5[value] === '0' ? false : true
        } else if (state === 'suggested_execution_time') {
          if (this.props.canUpdate || !this.props.show) {
            if (tsr5[value]) {
              if (tsr5[value] !== 'null') {
                obj[state] = moment(`${tsr5[value]}`, 'jYYYY/jM/jD')
              } else {
                obj[state] = undefined
              }
            }
          } else {
            obj[state] = tsr5[value] === 'null' ? '' : tsr5[value]
          }
        } else {
          obj[state] = tsr5[value]
        }
      }
      let ListMandatory = await ListSign.ListMandatory.tsr5
  ...
  handleSubmit = async () => {
    ...
    const CheckTo = notification_to
      ...
      : false
    const check = CheckTo && review_result ? true : handleCheckText(reject_msg)
    ...
    if (check) {
      ...
      await datareg.append('reject_msg', !review_result ? reject_msg : null)
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/tsr/tsr5/insert`,
        data: datareg,
        headers: {
          Authorization: this.state.token ? `Bearer ${this.state.token}` : null
        }
                ...
                <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
                  <div
                    className={`field-form persian textarea ${
                      this.state.foucs === `reject_msg` ||
                      handleCheckText(this.state.reject_msg)
                        ? 'active'
                        : ''
                    }`}
                  >
                    <div className='col p-0'>
                      ...
                      <textarea
                        ...
                        value={handleString(this.state.reject_msg)}
