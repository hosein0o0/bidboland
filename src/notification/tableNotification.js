...
export default class TableNotification extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      header: [
        { name: 'متن', value: 'title' },
        { name: 'تاریخ', value: 'date' },
        { name: 'ساعت', value: 'hour' }
      ],
      row: [],
      contentPerPage: 25,
      page: 1,
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/notification/archive/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
