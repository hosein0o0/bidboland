...
export default class TablePhonebook extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      header: [
        { name: 'ردیف', value: 'id' },
        { name: 'بخش', value: 'part' },
        { name: 'سمت', value: 'position' },
        { name: 'نام و نام خانوادگی', value: 'name' },
        { name: 'شماره داخلی', value: 'tel' }
      ],
      row: [],
      contentPerPage: 25,
      page: 1,
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/internalTel/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/internalTel/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/internalTel/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
                ...
                this.state.row.map((data, index) => (
                  ...
                  >
                    ...
                    {/* <td className='action justify-content-center'>
                                                {this.Permision.handlePermision(this.state.role, 'internal_tell_edit') &&
                                                    <Link className='mr-auto ml-auto' to={`#`} target='_blank' rel='noreferrer'>
                                                        <span className='edit'><EditRoundedIcon /></span>
                                                    </Link>
                                                }
                                            </td> */}
                  </tr>
              ...
              <div className='link-footer'>
                <a
                  href={OutputFilter.handleFilter(
                    ...
                    this.state.search,
                    `${StaticData.domainIp}/internalTel/exportExcel`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='xls'
                ...
                <a
                  href={OutputFilter.handleFilter(
                    this.state.textSearch,
                    this.state.search,
                    `${StaticData.domainIp}/internalTel/exportPdf`
                  )}
                  target='_blank'
                  rel='noreferrer'
                  className='pdf'
