...
export default class Address extends Component {
  ...
  handleChange = async e => {
    ...
    if (e.target.name === 'province') {
      await this.setState({ city: '', address: '', cityList: [] })
  ...
  handleSubmit = async () => {
    if (
      ...
      await datareg.append('province_id', this.state.province)
      await datareg.append('city_id', this.state.city)
      await datareg.append('address', this.state.address)
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/address/create`,
        data: datareg,
        headers: {
  ...
  provinceChange = async () => {
    if (this.state.token && this.state.province !== '') {
      await axios
        .get(
          `${StaticData.domainIp}/city/getCityListForSelectBox/${this.state.province}`,
          {
            headers: {
              Authorization: `Bearer ${this.state.token}`
            }
        ...
        .then(async response => {
          if (response.status === 200) {
            this.setState({ cityList: response.data.content })
          } else {
            Notification.notify(Message.text(response.status), 'error')
          }
        })
          ...
          <div className='main-form'>
            <div className='form row mt-0'>
              ...
              <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
                <div
                  className={`field-form persian pl-1 ${
                    this.state.foucs === `city` || this.state.city !== ''
                      ? 'active'
                      : ''
                  }`}
                >
              ...
              <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
                <div
                  className={`field-form persian textarea ${
                    this.state.foucs === 'address' || handleCheckText(this.state.address)
                      ? 'active'
                      : ''
                  }`}
                >
...
class Table extends Component {
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/address/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
              Authorization: `Bearer ${this.state.token}`
            }
  ...
  fetchData = async () => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/address/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`,
          {
            headers: {
              Authorization: `Bearer ${this.state.token}`
            }
