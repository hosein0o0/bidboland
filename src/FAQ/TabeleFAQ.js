...
export default class TabeleFAQ extends Component {
  constructor (props) {
    super(props)
    ...
    this.state = {
      ...
      header: [
        { name: 'عنوان ماژول', value: 'title' },
        { name: 'عنوان بخش اصطلاحات', value: 'idiom_title' },
        { name: 'توضیحات', value: 'text' }
      ],
      row: [],
      contentPerPage: 25,
      page: 1,
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/FAQ/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/FAQ/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/FAQ/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
  ...
  changeRowNumber = async e => {
    await this.setState({
      [e.target.name]: parseInt(handleString(e.target.value)),
      loading: 'table'
    })
            ...
            <div className='col-xl-5 col-lg-5 col-md-12 col-12 d-flex align-items-center'>
              {/* <div className='link-footer'>
                                <a
                                    ...
                                    //     this.state.search,
                                    //     `${StaticData.domainIp}/FAQ/exportExcel`)}
                                    target='_blank'
                                    rel='noreferrer'
                                    className='xls'>
                                ...
                                <a
                                    ...
                                    //     this.state.textSearch,
                                    //     this.state.search,
                                    //     `${StaticData.domainIp}/FAQ/exportPdf`)}
                                    target='_blank'
                                    rel='noreferrer'
                                    className='pdf'>
