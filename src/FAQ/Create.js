...
export default class CreateFAQ extends Component {
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - ایجاد FAQ`
  }
  OnFocus = name => {
  ...
  handleSubmit = async () => {
    ...
    const Ckeck = handleCheckText(title)
    if (Ckeck && this.props.location.state) {
      if (this.props.location.state.nameUrl) {
        await this.setState({ loading: 'submit', disabled: true })
        let datareg = await new FormData()
        await datareg.append('module_name', this.props.location.state.nameUrl)
        ...
        await axios({
          method: 'post',
          url: `${StaticData.domainIp}/FAQ/create`,
          data: datareg,
          headers: {
