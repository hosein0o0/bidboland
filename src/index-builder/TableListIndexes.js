...
export default class TableListIndexes extends Component {
  constructor (props) {
    super(props)
    ...
    this.state = {
      ...
      header: [
        ...
        { name: 'تعداد ستون', value: 'columnNo' },
        { name: 'فایل مربوطه', value: 'file_name' },
        { name: 'ایجاد کننده', value: '_author' }
      ],
      row: [],
      contentPerPage: 25,
      page: 1,
  ...
  handleSearch = async text => {
    if (this.state.token) {
      ...
      await axios
        .get(
          `${StaticData.domainIp}/dynamicIndex/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&searchInAll=${text}`,
          {
            headers: {
  ...
  fetchData = async (text = null) => {
    if (this.state.token) {
      ...
      let url = ''
      if (text !== null) {
        url = `${StaticData.domainIp}/dynamicIndex/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}&${text}`
      } else {
        url = `${StaticData.domainIp}/dynamicIndex/getList/${this.state.page}?contentPerPage=${this.state.contentPerPage}`
      }
  ...
  handleDelete = () => {
    if (this.state.rowDelete.id) {
      if (this.state.token) {
        this.setState({ loading: 'delete' })
        let datareg = new FormData()
        datareg.append('id', this.state.rowDelete.id)
        axios({
          method: 'post',
          url: `${StaticData.domainIp}/dynamicIndex/delete`,
          data: datareg,
          headers: {
          ...
          .then(async response => {
            if (response.status === 200) {
              await this.setState({ loading: '', rowDelete: '' })
              await this.fetchData()
            } else {
              await this.setState({ loading: '', rowDelete: '' })
              Notification.notify(Message.text(response.status), 'error')
          ...
          .catch(err => {
            this.setState({ loading: '', rowDelete: '' })
            if (err.response) {
              Notification.notify(Message.text(err.response.status), 'error')
            }
          })
                    ...
                    <td className='action justify-content-center'>
                      {this.Permision.handlePermision(this.state.role, '') ? (
                        <span
                          ...
                          onClick={() => this.setState({ rowDelete: data })}
                        >
                          <DeleteIcon />
                        </span>
                      ) : (
          ...
          >
            ...
            {this.state.rowDelete ? (
              <PopupConfirm
                ...
                close={e => this.setState({ rowDelete: e })}
                number={this.state.rowDelete.title}
                label={'ایندکس'}
