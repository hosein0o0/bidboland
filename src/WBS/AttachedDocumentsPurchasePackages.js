...
export default class AttachedDocumentsPurchasePackages extends Component {
  constructor (props) {
    super(props)
    ...
    this.state = {
      ...
      totalAdvance: 0,
      secondData: {},
      baseUrlAPI: 'purchasePackage/attachmentList'
    }
  }
  componentDidMount () {
  ...
  handleSearch = async text => {
    ...
    if (token) {
      ...
      let url = `${
        StaticData.domainIp
      }/${baseUrlAPI}/attachmentList/${page}?contentPerPage=${contentPerPage}${
  ...
  _handleClick = async (key, data, checkAdd) => {
    ...
    if (token) {
      ...
      let url = `${
        StaticData.domainIp
      }/${baseUrlAPI}/attachment/getAutoFill/${nameDataBase}/page/${pageAdvance}?searchInAll=${search}&${
  ...
  handleSearchAdvance = async text => {
    ...
    if (checkheader) {
      ...
      if (token) {
        ...
        let url = `${
          StaticData.domainIp
        }/${baseUrlAPI}/attachment/getAutoFill/${nameDataBase}/page/${pageAdvance}
    ...
    return (
      <React.Fragment>
        ...
        <FooterTable
          ...
          xlsName='purchase_attachment_package_export'
          pdf={`${baseUrlAPI}/exportPdf`}
          pdfName='purchase_attachment_package_export'
        />
      </React.Fragment>
    )
  }
