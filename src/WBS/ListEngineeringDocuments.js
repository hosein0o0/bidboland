...
export default class ListEngineeringDocuments extends Component {
  constructor(props) {
    super(props)
    ...
    this.state = {
      ...
      header: [
        ...
        { name: 'Doc Code1', value: 'doc_code1' },
        { name: 'Doc Code2', value: 'doc_code2' },
        ...
        { name: 'Last Plan P.O.I', value: 'last_plan_poi' },
        { name: 'First Step Plan Date', value: 'first_step_plan_date' },
      ...
      totalAdvance: 0,
      secondData: {},
      baseUrlAPI: 'EngineeringDocumentList'
    }
  }
  componentDidMount() {
  ...
  handleSearch = async text => {
    ...
    if (token) {
      ...
      let url = `${StaticData.domainIp}/${baseUrlAPI}/getList/${page}?contentPerPage=${contentPerPage}${handleCheckText(text) ? `&searchInAll=${text}` : ''}${handleCheckText(textSearch) ? `&searchByFields=${textSearch}` : ''}`
      fetchDataAPI(url)
    }
  }
  fetchData = async (text = null) => {
    ...
    if (token) {
      let url = `${StaticData.domainIp}/${baseUrlAPI}/getList/${page}?contentPerPage=${contentPerPage}${handleCheckText(search) ? `&searchInAll=${search}` : ''}${handleCheckText(text) ? `&${text}` : ''}`
      fetchDataAPI(url)
    }
  }
  changeRowNumber = e => {
    ...
    return (
      <React.Fragment>
        ...
        <FooterTable
          ...
          xlsName='engineering_document_list_export'
          pdf={`${baseUrlAPI}/exportPdf`}
          pdfName='engineering_document_list_export'
        />
      </React.Fragment>
    )
  }
