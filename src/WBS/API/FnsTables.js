...
class FnsTables {
  ...
  VitualRealityLink = (data) => {
    const check = StaticData.domainIp === 'http://185.105.239.21:8083/185.105.239.21:8080'
    const { unit_system, tag_no } = data
    let result = { url: null }
    if (check) {
      if (handleCheckText(unit_system) && handleCheckText(tag_no)) {
        let url = `http://185.105.239.21:8083/185.105.239.21:9090/PDF/${handleString(unit_system)}/${handleString(tag_no)}.pdf`
        const check_url = CheckDownload(url)
        result = { url: check_url ? url : null }
      }
    } else {
      let url2 = `${StaticData.link_file}/upload/wbs/virtual_reality/${tag_no}.pdf`
      const check_url2 = CheckDownload(url2)
      result = { url: check_url2 ? url2 : null }
    }
    return result
  }
