...
export default class EditSecond extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    document.title = `${StaticData.Title} - ویرایش مدیریت جلسه و صورت جلسه نهایی`
  ...
  fetchData = id => {
    if (this.state.token) {
      axios
        .get(`${StaticData.domainIp}/meeting/getPart2Detail/${id}`, {
          headers: {
        ...
        .then(response => {
          if (response.status === 200) {
            let ReasonRejection = {
              reject_at: response.data.content.reject_at,
              reject_by: response.data.content.reject_by,
              reject_sign_message: response.data.content.reject_sign_message
            }
            let state = response.data.content.firstDetail
            state['ReasonRejection'] = ReasonRejection
            response.data.content.selected_attendees.forEach(data => {
              state[`attendees_${data.value}`] = true
            })
            response.data.content.signature.forEach(data => {
              state[`signatories_${data.value}`] = true
            })
            this.setState(state)
          } else {
  ...
  handleSubmit = async () => {
    ...
    for (let value in this.state) {
      ...
      } else if (value.includes('signatories_')) {
        if (this.state[value]) {
    ...
    if (check && attendees.length > 0 && signatories.length > 0) {
      ...
      await axios({
        method: 'post',
        url: `${StaticData.domainIp}/meeting/updateSecondStep`,
        data: datareg,
        headers: {
                    ...
                    <div className='boxes-dashboard row m-0 p-0'>
                      <div className='main-form'>
                        <div className='title-from'>
                          <h2>ویرایش مدیریت جلسه و صورت جلسه نهایی</h2>
                        </div>
                        <div className='col-xl-8 col-lg-10 col-md-12 col-12'>
                          {this.state.ReasonRejection.reject_at &&
                          this.state.ReasonRejection.reject_by &&
                          this.state.ReasonRejection.reject_sign_message ? (
                            <div className='col-12'>
                              <div className='message-error'>
                                <label className='row mr-0 ml-0'>
                                  صورت جلسه توسط
                                  ...
                                  به دلیل زیر تایید نشد :
                                </label>
                                <p className='m-0'>
                                  {
                                    this.state.ReasonRejection
                                      .reject_sign_message
                                  }
                                </p>
                              </div>
                              ...
                              >
                                ...
                                <div className='col-xl-12 col-lg-12 col-md-12 col-12'>
                                  <div className={`field-form persian`}>
                                    ...
                                    <div className='pl-1 allName col row m-0 justify-content-end'>
                                      {this.Convertor(meeting.attachment)
                                        .length === 0 ? (
                                ...
                                <div className='col-xl-6 col-lg-6 col-md-12 col-12'>
                                  ...
                                  >
                                    ...
                                    <input
                                      readOnly={true}
                                      value={handleString(meeting.responsible)}
                                    />
                                  </div>
                                </div>
                                <div className='col-xl-6 col-lg-6 col-md-12 col-12'>
                                ...
                                <div className='col-xl-6 col-lg-6 col-md-12 col-12'>
                                  <div
                                    className={`field-form persian ${
                                      ...
                                      handleCheckText(meeting.acting)
                                        ? 'active'
                                        : ''
                                    }`}
                                  >
                                    ...
                                    <input
                                      readOnly={false}
                                      name={`acting__${key1}`}
                                      value={handleString(meeting.acting)}
